...
function App() {
    ...
    const isIngredientCardModalOpen = JSON.parse(
        localStorage.getItem('modalIngredientCard')
    )

    useEffect(() => {
        dispatch(fetchDataAction(INGREDIENTS_URL))
        if (isIngredientCardModalOpen)
            dispatch(openCardModal(isIngredientCardModalOpen))
    }, [])
    ...
    return (
        <div className={appStyles.app}>
            ...
            <Routes location={state?.background || location}>
                ...
                />
                {!state?.background && (
                    <Route path="/ingredients/:id" element={<IngPage />} />
                )}
