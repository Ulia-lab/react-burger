...
const Card = ({ card }) => {
    ...
    const constructorCards = useSelector(
        (state) => state.constructorItems.items
    )
    const count = useMemo(
        () => constructorCards.filter((item) => item._id === card._id).length,
        [card._id, constructorCards]
    )
        ...
        <div ref={dragRef}>
            ...
            >
                <Link
                    ...
                    className={burgerIngredientsStyle.link}
                >
                    {count !== 0 && (
                    ...
                    <p
                        className={cn(
                            'text text_type_main-small mt-1',
                            burgerIngredientsStyle.name
                        )}
                    >
                        {card.name}
                    </p>
