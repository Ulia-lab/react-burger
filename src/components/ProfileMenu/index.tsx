...
export function ProfileMenu({ onClick }: ProfileMenuProps) {
    const menuItems = [{ title: 'Профиль', link: '/profile' }, { title: 'История заказов', link: '/profile/orders' }];
    return (
        <div className={menuStyle.block}>
            {menuItems.map((item, index) => <NavLink to={item.link} key={index} className={cn("text text_type_main-medium", menuStyle.item)}>{item.title}</NavLink>)}
